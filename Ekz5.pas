...
var
  i, j: integer;
begin
  writeln('Введите все элементы матрицы через пробел');
  for i := 1 to n do
  begin
    for j := 1 to n do
    begin
...
begin
  ...
  proverka:=true;
  for j := 1 to n do
    sumdiag1 := sumdiag1 + a[j, j];{Нашли сумму всех элементов первой диагонали}
  for i := 1 to n do
  begin
    s1 := 0;
    s2 := 0;
  ...
  if (proverka = true) then writeln('Матрица является магическим квадратом')
